...
class BeneficiaryController extends Controller
{
    ...
    /**
     * Export beneficiaries list to CSV based on current filters.
     */
    public function export(Request $request): \Symfony\Component\HttpFoundation\StreamedResponse
    {
        $query = Beneficiary::with(['office', 'card', 'creator', 'user'])
        ...
        $filename = 'SECURE_4Ps_Beneficiaries_' . now()->format('Ymd_His') . '.csv';

        AuditLogService::log('beneficiaries_exported', null, [], ['count' => $beneficiaries->count()], 'Exported beneficiary list to CSV');
        ...
        return response()->streamDownload(function () use ($beneficiaries) {
            ...
            fputcsv($handle, [
                'Household ID (Unique ID)', 'Listahanan ID', 'Last Name', 'First Name', 'Middle Name', 'Suffix',
                'Sex', 'Birthdate', 'Civil Status', 'Contact Number', 'Barangay', 'Full Address',
                'City', 'Province', 'Status', 'Family Members Count', 'Card Status', 'Portal Username', 'Registered Date'
            ]);

            foreach ($beneficiaries as $b) {
                $cardStatus = $b->card ? 'Issued' : 'No Card';
                $address    = implode(', ', array_filter([$b->house_no, $b->street, $b->purok]));
                ...
                fputcsv($handle, [
                    ...
                    $b->suffix ?? '',
                    ucfirst($b->sex ?? ''),
                    $b->birthdate ? $b->birthdate->format('Y-m-d') : '',
                    ucfirst($b->civil_status ?? ''),
                    $b->contact_number ?? '',
                    $b->barangay,
                    $address,
                    $b->city ?? 'Lipa City',
